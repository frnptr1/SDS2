...
```{r}
cbind(bayesian_pc, p_c, bayesian_pt, p_t)
...
```{r}
cbind(bayesian_pc2, p_c, bayesian_pt2, p_t)
```
...
```{r}
lr  = lm(data$rt ~ data$nt)
lr2 = lm(data$rc ~ data$nc)


plot(x=data$nt, y=data$rt, main = "Linear Regression with empirical data", xlab = "Sample size", ylab = "Deaths", pch =16, col ="red1", cex = 1.5)
points(x = data$nc, y = data$rc, pch = 16, col= "royalblue1", cex = 1.5)
abline(lr, col = "red3", lwd = 2)
abline(lr2, col = "royalblue3", lwd = 2)
legend("bottomright", legend = c("Treatment", "Control"), col = c("red1","royalblue1"), pch = c(16,16), pt.cex = .6)
grid()
```
...
crude_delta_estimation = logit(coefficients(lr)[2])-logit(coefficients(lr2)[2])
estimates = cbind(crude_delta_estimation, Model1$BUGSoutput$summary[1,1], Model2$BUGSoutput$summary[1,1])
...
abline(v=crude_delta_estimation, col = "orange", lwd = 1.5)
legend("topright", legend = c("Lower/Upper hdi", "Frequentist estimate"), col = c("blue", "orange"), lty = c(1, 1), lwd = c(1.5, 1.5), cex = 0.6)
grid()
```

```{r}
#mix = glmer(dat$deaths/dat$patients ~ dat$patients + (1|dat$group), weights = dat$patients, family = binomial)
```
...
a = lm(c(logit_pt, logit_pc)~group)
b = lm(c(p_t, p_c)~group)
```
...
```{r}
plot(x = as.factor(group), y = c(logit_pt, logit_pc), type = "l")
abline(a)
```
