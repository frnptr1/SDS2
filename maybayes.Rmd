...
model3 = "
        ...
        {
          for( i in 1 : Num ) {
            rc[i] ~ dbinom(pc[i], nc[i])
            rt[i] ~ dbinom(pt[i], nt[i])
            pc[i] ~ dbeta(1,1)
            pt[i] ~ dbeta(1,1)
        }   
      }"

writeLines( model3 , con="m3" )

data3 = list(rt = c(3, 7, 5, 102, 28, 4, 98, 60, 25, 138, 64, 45, 9, 57, 25, 33, 28, 8, 6, 32, 27, 22 ),
            nt = c(38, 114, 69, 1533, 355, 59, 945, 632, 278,1916, 873, 263, 291, 858, 154, 207, 251, 151, 174, 209, 391, 680),
            rc = c(3, 14, 11, 127, 27, 6, 152, 48, 37, 188, 52, 47, 16, 45, 31, 38, 12, 6, 3, 40, 43, 39),
            nc = c(39, 116, 93, 1520, 365, 52, 939, 471, 282, 1921, 583, 266, 293, 883, 147, 213, 122, 154, 134, 218, 364, 674),
            Num = 22
            )
#init3= function() list("a"=1, "b"=1 )
 

params = c("pc", "pt")
  
Model3 = jags( model.file="m3" , data=data3, 
                n.chains=2,parameters.to.save = params ,
                n.iter =20000,n.thin=1,n.burnin = 2000)
Model3

out = Model3$BUGSoutput$sims.array[,1,]

a = colMeans(out, dims = 1)[-1]

estimates_pc = mean(a[1:22])
estimates_pt = mean(a[22:44])
```
...
blocker<-read.csv("./Scaricati/BLOCKER.csv", sep=",", header=T)
Nstud<-22
